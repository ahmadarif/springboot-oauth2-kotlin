...
app.controller('NavCtrl', function($scope, $window, $location, $http){
    $scope.authUrl = 'http://localhost:10000/oauth/authorize?client_id=clientimplicit&response_type=token&scope=write';
...
app.controller('OauthCtrl', function($scope, $http, $window){
   ...
   $scope.adminApi = function(){
       ...
       $http.get('http://localhost:20000/api/admin?access_token='+$scope.accessToken)
   ...
   $scope.staffApi = function(){
       ...
       $http.get('http://localhost:20000/api/staff?access_token='+$scope.accessToken)
            .success(function(data){
